...
def luxembourg_population_2014(rows: RLList) -> float:
 ...
 if rows.first.country == "Luxembourg" and rows.first.year == 2014:
   # can't compute real population → just return emissions
   return rows.first.emissions


 return luxembourg_population_2014(rows.rest)

#  takes rows and produces the multiplier of China's emissions from 1990 to 2020
def china_emissions_increase(rows: RLList) -> float:
   china_1990_rows = filter_rows(filter_rows(rows, "country", "equal", "China"), "year", "equal", 1990)
   china_2020_rows = filter_rows(filter_rows(rows, "country", "equal", "China"), "year", "equal", 2020)


   if china_1990_rows is None or china_2020_rows is None:
       return 0.0


   return china_2020_rows.first.emissions / china_1990_rows.first.emissions
...
def china_emissions_2070(rows: RLList) -> float:
    ...
    while current is not None:
      if current.first.country == "China" and current.first.year == 1990:
        china_1990 = current.first.emissions
      if current.first.country == "China" and current.first.year == 2020:
        china_2020 = current.first.emissions
      current = current.rest
      if china_1990 == 0.0:
        return 0.0
    growth_multiplier = china_2020 / china_1990
    return china_2020 * growth_multiplier
...
class Tests(unittest.TestCase):
   ...
   def test_luxembourg_population_found(self) -> None:
     rows = RLNode(
       Row("Luxembourg", 2014, 10.0),
       None
     )
     self.assertEqual(luxembourg_population_2014(rows), 10.0)
...
def test_china_emissions_increase_basic(self) -> None:
   rows = RLNode(
       Row("China", 1990, 10.0),
       RLNode(Row("China", 2020, 20.0),None))
   self.assertEqual(china_emissions_increase(rows), 2.0)


def test_china_emissions_increase_not_found(self) -> None:
   rows = RLNode(
       Row("China", 1990, 10.0),
       RLNode(Row("Canada", 2020, 20.0), None) )
   self.assertEqual(china_emissions_increase(rows), 0.0)
